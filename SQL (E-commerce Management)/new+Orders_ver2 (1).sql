...
select
    ...
    CASE WHEN SUM(oi.PRODUCT_QUANTITY) = 0 THEN 'No Sales in past, give discount to reduce inventory'
			...
			WHEN SUM(p.PRODUCT_QUANTITY_AVAIL) < 0.5*(SUM(oi.PRODUCT_QUANTITY)) THEN 'Medium inventory, need to add some inventory'
...
group by p.PRODUCT_ID, pc.PRODUCT_CLASS_DESC
...
select
    ...
    (SELECT 
		CASE 
			...
			WHEN SUM(p.PRODUCT_QUANTITY_AVAIL) < 0.6*(SUM(oi.PRODUCT_QUANTITY)) THEN 'Medium inventory, need to add some inventory'
...
select
    ...
    (SELECT 
		CASE 
			...
			WHEN SUM(p.PRODUCT_QUANTITY_AVAIL) < 0.7*(SUM(oi.PRODUCT_QUANTITY)) THEN 'Medium inventory, need to add some inventory'
